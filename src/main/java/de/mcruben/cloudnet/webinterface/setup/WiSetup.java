package de.mcruben.cloudnet.webinterface.setup;
    ...
    public WiSetup() {
        setupComplete(document -> {
...

            {
                ...
                if (file.exists())
                    WebInterface.getInstance().deleteDirectory(file);

                file.mkdir();

                try {
                    String finalDomain = domain;
                    readAndWriteConfig(updateConfigurations, "Website/config.php", input -> {
                        if (input.equals("%server_name%"))
                            return "$servername = \"" + serverName + "\";";
                        if (input.equals("%domain%"))
                            return "$domain = \"" + finalDomain + "\";";
                        if (input.equals("%ip%"))
                            return "$ip = \"" + ip + "\";";
                        if (input.equals("%webport%"))
                            return "$webport = \"" + webport + "\";";
                        if (input.equals("%token%"))
                            return "$cloudnettoken = \"" + token + "\";";
                        if (input.equals("%user%"))
                            return "$cloudnetuser = \"" + userName + "\";";
                        if (input.equals("%bungeeip%"))
                            return "$bungeeip = \"" + bungeeip + "\";";
                        if (input.equals("%bungeeport%"))
                            return "$bungeeport = \"" + bungeeport + "\";";
                        if (input.equals("%version%"))
                            return "$version = \"" + WebInterface.getInstance().getModuleConfig().getVersion() + "\";";
                        return input;
                    });
                } catch (IOException e) {
                    e.printStackTrace();
