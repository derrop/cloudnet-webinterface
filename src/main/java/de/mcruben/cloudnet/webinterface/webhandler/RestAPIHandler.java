package de.mcruben.cloudnet.webinterface.webhandler;
...
import de.mcruben.cloudnet.webinterface.objects.AdvancedWrapperInfo;
...
public class RestAPIHandler extends WebHandler {
    ...
    public FullHttpResponse handleRequest(ChannelHandlerContext channelHandlerContext, QueryDecoder queryDecoder, PathProvider pathProvider, HttpRequest httpRequest) throws Exception {
        ...
        if (WebInterface.getInstance().isDebug()) {
            System.out.println("[WebInterface] HttpRequest from " + channelHandlerContext.channel().remoteAddress() + " (message: " + message + "; value: " + value + ")");
        }
        ...
        switch (message.toLowerCase()) {
            ...
            case "corelog":
            {
                if (!user.hasPermission("web.console"))
                    return noPermission(httpRequest);
                responseDocument.append("success", true).append("log", WebInterface.getInstance().getCorelog().toString());
            }
            break;
            ...
            case "debug":
            {
                responseDocument.append("success", true).append("response", WebInterface.getInstance().isDebug());
            }
            break;
            ...
            case "config":
            {
                JsonElement config = WebInterface.getInstance().getConfigManager().getConfig(value);
                if (config == null) {
