...
header('Location: '.$domain.'/logged/index.php?erfolg=stopproxy&#2');
...
if($pagestyle == 2) {
	if(isset($_SESSION['Logged'])) {
		...
		$jsonlol = file_get_contents($url, false, $context);
		$json = json_decode($jsonlol);
		header('Location: '.$domain.'/logged/index.php?set=proxy&erfolg=stopproxy&#2');
				//erfolg
    } else {
		header('Location: '.$domain.'/index.php?error=403');
//error
	...
	if(isset($_SESSION['Logged'])) {
		...
		$jsonlol = file_get_contents($url, false, $context);
		$json = json_decode($jsonlol);
		header('Location: '.$domain.'/logged/index.php?erfolg=stopproxy&#5');
				//erfolg
    } else {
		header('Location: '.$domain.'/index.php?error=403');
//error
